...
改善各种调试窗口的标题字符串变量：新增`Lang_DebugWindow_Retrieving_Caption`变量，删除掉多余的变量。因为该字符串是可以共用的。

frmCodeWindow的Form_GotFocus事件会令文本框获得焦点。

编写“模块”调试窗口的代码。

编写“线程”调试窗口的代码。

把强制结束进程的退出码从-1改成了0。其实没啥影响。感觉改成0会漂亮点。我知道没人会理这个细微的东西。也没必要理会。我也知道根本就没人会来看commit记录或者这个文件。也不会有人真的去读这个repo里的代码。这个repo唯一吸引人的地方就是主页摆的那几张我很久没有更新过的图片。难道不是吗。每次commit都写的这个记录，其实到头来连我自己都不会一行行细读。好吧..我怎么会奢望有人会认真读这个文件呢。谁想看这么多文字呢。一看到就烦。这行文本也不会有人看到，不是吗。到头来只不过是我发发牢骚浪费时间打的一行愚蠢的文本罢了。

# 2020.4.4

今天是清明节。这个清明节比以往的清明节更为沉重，因为在这几个月内许许多多的人都在与新型冠状病毒肺炎抗战的过程中失去了宝贵的生命。许多前线的医护人员前仆后继，勇敢地面对着这种可怕的病毒，谱写了一个个可歌可泣的故事。不仅仅是医护人员，可以说在任何岗位上坚守的工作者都是伟大的，都是值得歌颂的。他们都是抗疫英雄，他们冒着生命危险为他人服务。国内的疫情形势逐渐平缓，而国外的抗疫之战则打得水深火热。在这场疫情战面前，我们不分名族，不分国籍，无论男女老少，都是其中的战士。让我们坚强地共同奋斗，一同打赢这场抗疫之战！在这个沉重的日子里，我衷心希望：愿逝者安息，愿生者奋发，愿祖国昌盛，致敬抗疫英雄！人类加油，世界加油！

错误列表窗口进行大幅修改，取消原有的绘制图标方法，改成使用系统原生的ListView设置图标的方式。首先，程序创建一个图像列表（ImageList），然后从资源文件读取图标并添加到图像列表里，接着添加ListView列表项的时候设置新添加的列表项的图标。这样做的好处是：1. 精简代码; 2. 避免了不必要的子类化; 3. 大幅优化性能; 4. 让程序更加稳定。

frmMain的Form_Initialize事件中指定启动窗口后取消置顶。置顶的目的是让用户能够看到这个窗口，取消置顶的原因是它可能会挡住一些消息框（如加载语言失败的错误提示）。
...
修复本地变量窗口AddVarItem函数存储时有时会添加多余的空行的问题。

处理本地变量窗口的StringParser函数有时候会向gdb发送无效命令。修改之后的代码如果检测到gdb的输出是“A syntax error in expression”就直接放弃parse。

优化本地变量窗口的获取变量类型方式，把一些使用`ptype`命令的地方改成使用`whatis`。因为这样能够避免大篇幅地输出struct/class等类型的变量。

优化本地变量窗口的获取变量代码，添加获取超时，防止程序卡死。

添加了主窗体的计时器里处理程序抛出异常的代码。当程序抛出异常后，IDE会进入中断状态，并像有断点命中一样处理（例如输出调用堆栈，显示本地变量等）。

补充frmOutput的AddLineInfo的函数注释。发现漏了一个参数说明...

修改本文件里一些不准确的用词。（详情自己看commit...）

为人类祈祷。为地球祈祷。人类加油！

# 2020.1.26

为显示代码窗口专门编写了一个过程：frmMain.ShowCodeWindow。该过程能根据文件名或者文件序号来切换到对应的代码窗口。因为许多地方都需要这个操作，所以这样可以让代码不这么臃肿。

修改一些错别字（“断的”→“断点”, “断电”→“断点”）

修复了中断模式下代码窗口的断点会被小箭头遮盖的问题。

把代码框设置成Tab用空格替代，因为这样才能准确计算字符的位置（Tab被视为1个字符，但是g++却认为他是4个空格）。

编写了错误列表窗口的代码。

修复frmLocals的BracketsParser中的一个错误。

把调整frmMain的主菜单的状态的操作单独放到一个过程里，按照不同的调试状态来更改菜单的状态。

为frmOutput的文本框增加双击响应功能，包括双击exe路径能够在文件浏览器里打开；双击错误消息能跳转到对应的代码行等。

修复弹出菜单窗口响应快捷键时忽略菜单项已禁用而继续触发对应过程的bug。

整理图标目录。

（啰嗦话）最近新型冠状病毒好恐怖啊... 真心希望大家都好好的。希望早日找到有效的疗法！世界加油！人类加油！❤
